...
class UPGD_DynamicclippedGradient(torch.optim.Optimizer):
    def __init__(self, params, lr=1e-5, weight_decay=0.0, beta_utility=0.0, sigma=1.0):
        ...
        super(UPGD_DynamicclippedGradient, self).__init__(params, defaults)
    ...
    def step(self):
        ...
        for group in self.param_groups:
            for name, p in zip(group["names"], group["params"]):
                ...
                # Dynamically compute the clipping threshold
                grad_norm = torch.norm(p.grad.data) + 1e-8
                clipping_threshold = group.get("clipping_factor", 2.0) * grad_norm.median().item()

                # Clip gradients dynamically
                clipped_grad = torch.nn.utils.clip_grad_norm_([p], max_norm=clipping_threshold)

                # Apply weight decay and parameter update
                p.data.mul_(1 - group["lr"] * group["weight_decay"]).add_(
                    (clipped_grad + noise) * (1 - scaled_utility),
                    alpha=-group["lr"],
                )
