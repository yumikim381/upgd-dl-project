...
class RunStats:
    ...
    def start(self):
        ...
        criterion = (
            extend(criterions[self.task.criterion]())
            if self.learner.extend
            else criterions[self.task.criterion]()
        )
        ...
        with tqdm(total=self.n_samples, desc="Training Progress", unit="step") as pbar:
            for i in range(self.n_samples):
                ...
                if self.task.criterion == "cross_entropy":
                    accuracy_per_step.append(
                        (output.argmax(dim=1) == target).float().mean().item()
                    )
                ...
                with torch.no_grad():
                    ...
                    plasticity_per_step.append(
                        torch.clamp(
                            (1 - loss_after / loss_before), min=0.0, max=1.0
                        ).item()
                    )
                n_dead_units = 0
                ...
                for _, value in self.learner.network.activations.items():
                    n_dead_units += value
                n_dead_units_per_step.append(
                    n_dead_units / self.learner.network.n_units
                )
                ...
                for name, param in self.learner.network.named_parameters():
                    if "weight" in name:
                        if "conv" in name:
                            sample_weight_rank += (
                                torch.torch.linalg.matrix_rank(param.data)
                                .float()
                                .mean()
                            )
                            sample_max_rank += torch.min(
                                torch.tensor(param.data.shape)[-2:]
                            )
                        else:
                ...
                weight_rank_per_step.append(
                    sample_weight_rank.item() / sample_max_rank.item()
                )

                if i % self.task.change_freq == 0:
                    losses_per_task.append(sum(losses_per_step) / len(losses_per_step))
                    if self.task.criterion == "cross_entropy":
                        accuracy_per_task.append(
                            sum(accuracy_per_step) / len(accuracy_per_step)
                        )
                    plasticity_per_task.append(
                        sum(plasticity_per_step) / len(plasticity_per_step)
                    )
                    n_dead_units_per_task.append(
                        sum(n_dead_units_per_step) / len(n_dead_units_per_step)
                    )
                    weight_rank_per_task.append(
                        sum(weight_rank_per_step) / len(weight_rank_per_step)
                    )
                    weight_l2_per_task.append(
                        sum(weight_l2_per_step) / len(weight_l2_per_step)
                    )
                    weight_l1_per_task.append(
                        sum(weight_l1_per_step) / len(weight_l1_per_step)
                    )
                    grad_l2_per_task.append(
                        sum(grad_l2_per_step) / len(grad_l2_per_step)
                    )
                    grad_l1_per_task.append(
                        sum(grad_l1_per_step) / len(grad_l1_per_step)
                    )
                    grad_l0_per_task.append(
                        sum(grad_l0_per_step) / len(grad_l0_per_step)
                    )
                ...
                if i % 100000 == 0 and i != 0:
                    try:
                        self.save_model(
                            f"/work/scratch/yumkim/model_{self.learner.name}_{self.task_name}_{i}.pth"
                        )
                        print(
                            f"Saving the results into /work/scratch/yumkim/model_{self.learner.name}_{self.task_name}_{i}.pth"
                        )
            ...
            self.save_model(
                f"/work/scratch/yumkim/model_{self.learner.name}_{self.task_name}_final.pth"
            )
            if self.task.criterion == "cross_entropy":
                self.logger.log(
